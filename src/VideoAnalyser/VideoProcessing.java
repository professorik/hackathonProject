...
public class VideoProcessing {
    ...
    private void initOpenCv(File file) {
        videoCapture = new VideoCapture(file.getAbsolutePath());
        //videoCapture.open(file.getAbsolutePath());
        Size frameSize = new Size((int) videoCapture.get(Videoio.CAP_PROP_FRAME_WIDTH), (int) videoCapture.get(Videoio.CAP_PROP_FRAME_HEIGHT));
        videoWriter = new VideoWriter("D://result/"+file.getName().split("\\.")[0]+".mp4", VideoWriter.fourcc('M', 'P', '4', 'V'), videoCapture.get(Videoio.CAP_PROP_FPS), frameSize, true);
        Writer.println("Video is open: " + videoCapture.isOpened(), 35);
    }
    ...
    public void convert(File file){
        ...
        Mat mat = new Mat();
        MatOfRect matOfRect;
        CascadeClassifier detector = DataProcessor.face_detector_def;
        while (videoCapture.isOpened()) {
            videoCapture.read(mat);
    ...
    public static void main(String[] args) {
        ...
        new VideoProcessing().convert(new File("D:\\IdeaProjects\\hackathonProject\\src\\resources\\video.mp4"));
    }
}
